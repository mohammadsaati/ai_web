...
if(!function_exists('menu'))
{
    function menu()
    {
        $menu = new \App\Classes\Theme\Menu(items: config("main_menu")["items"]);

        return $menu;
    }
}

if (!function_exists("slug_creator"))
{
    function slug_creator(string $class_name ,  string $title , string $field_name = "slug") : string
    {
        $slug = new \App\Classes\Slug\Slug(class_name :$class_name , field_name : $field_name , title : $title);
...
{
    function file_name($file) : string
    {
        return Str::ulid()->toBase32().time().".".$file->getClientOriginalExtension();
    }
}

if(!function_exists("image_creator"))
{
    function image_creator($path , $file_name , $image_input = "image") : void
    {
