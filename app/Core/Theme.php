...
class Theme
{
     ...
     * @return string
     */
    static function printHtmlAttributes($scope)
    {
        $attributes = [];
     ...
     * @return string
     */
    static  function printHtmlClasses($scope, $full = true)
    {
    ...
    public static function getPageUrl($path, $demo = '', $mode = null)
    {
        ...
        if (Route::has($name)) {
            return Theme . phproute($name) . $a;
        }

        // otherwise return as url
        return Theme . phpurl($path) . $a;
    }
    ...
    {
        return sprintf('<link rel="shortcut icon" href="%s" />', asset(config('settings.title_logo')));
    }
